...
import { shieldTokens, fakeshield } from "./src/transactions/shield";
import { isEvmAddress } from "./src/transactions/adresses";
import SHIELD_CONTRACT_ADDRESS from "./src/transactions/shield";
import fakeerc20asset from "./src/transactions/shield";
import { make_deposit_tx, gen_tx_no_sig } from "./src/transactions/txgen";
import { unshieldTokens, fetchKzgParams } from "./src/transactions/unshield";
import { generate_tx2, xcm_chains } from "./src/transactions/xcm";
import { westend_pool, generateCommitment, zkDeposit, zkWithdraw } from "./src/transactions/zkg16";
...
const NETWORKS = {
  ...
  paseo_assethub2: {
    ...
    shield_address: "0xEf4324776f54Cc8Cb1397c2475300A4a2AddE3Df",
    abi: [
      "function deposit(address, uint256, bytes32, bytes32) payable",
      "function withdraw(uint256[2], uint256[2][2], uint256[2], uint256[8])"
...
paseo_assethub: {
  ...
  kusama: {
    ...
    rpcEndpoint: "https://kusama-asset-hub-eth-rpc.polkadot.io",
    asset: "KSM",
    alternative_assets: [
	{
	name: "DOT",
...
export function App() {
  ...
  const [activeTab, setActiveTab] = useState<"shield" | "unshield" | "bridge" | "crosschainbridge">(
    "shield",
  );
...
<option value="westend_assethub">
          ...
          {activeTab === "crosschainbridge" ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "200px" }}>
              <button
                ...
                style={{ maxWidth: 220, margin: "2rem auto", fontSize: "1.2rem", opacity: 0.7, cursor: "not-allowed" }}
                disabled
              >
                🚧 Coming Soon 🚧
              </button>
  ...
  {/* Alternative assets */}
  {NETWORKS[selectedNetwork].alternative_assets?.map((token) => (
    <option 
      key={token.name}
...
{activeTab === "shield" && NETWORKS[selectedNetwork].faucet && (
              ...
              {/* Bottom tab bar */}
              <div className="tabs bottom-tabs" style={{ marginTop: "2rem", borderTop: "1px solid rgba(147, 51, 234, 0.2)", justifyContent: "center" }}>
                <button
