...
async function main() {
  const testNetworks = ["paseo_assethub", "westend_assethub", "polkadot", "kusama", "moonbase"];
  ...
  console.log(`  Same inputs yield same result: ${a === b ? "PASS" : "FAIL"} (${a} vs ${b})`);
  ...
  for (const network of testNetworks) {
    ...
    console.log(`  ${network}: unshield/shield ratio = ${ratio.toFixed(2)}x (${shieldCost} vs ${unshieldCost})`);
  }
  ...
  const recentCostVal = await estimateGasCost("paseo_assethub", "shield", 10000n);
  const smaller = parseFloat(recentCostVal) < parseFloat(defaultCost);
  console.log(`  Default: ${defaultCost} PAS, Recent (10k): ${recentCostVal} PAS`);
